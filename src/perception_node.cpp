      ...
      is_subscribed_(false)
{
  ...
  odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
      ...
      std::bind(&PerceptionNode::odomCallback, this, std::placeholders::_1));
      
  imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
      "/" + drone_namespace_ + "/imu", 10,
      std::bind(&PerceptionNode::imuCallback, this, std::placeholders::_1));
  ...
  detect_pub_ = this->create_publisher<apriltag_msgs::msg::AprilTagDetectionArray>(
      "/" + drone_namespace_ + "/tags", 10, pub_options);
      
  scenario_detection_pub_ = this->create_publisher<std_msgs::msg::String>(
      "/" + drone_namespace_ + "/scenario_detection", 10);
  ...
  RCLCPP_INFO(this->get_logger(), "AprilTag detector configured - Family: %s, Threads: %d", 
              get_parameter_or("tag_family", std::string("tf36h11")).c_str(),
              get_parameter_or("num_threads", 1));
}
...
void PerceptionNode::subscribe()
{
  ...
  front_image_sub_ = std::make_shared<image_transport::Subscriber>(
      image_transport::create_subscription(
          this, "/" + drone_namespace_ + "/front/image",
...
void PerceptionNode::frontImageCallback(const sensor_msgs::msg::Image::ConstSharedPtr& msg)
{
  ...
  RCLCPP_DEBUG(this->get_logger(), "Processed frame - Total messages: %zu, Total tags: %zu", 
               num_messages_, num_tags_detected_);
}
...
void PerceptionNode::odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg)
{
  ...
  RCLCPP_DEBUG(this->get_logger(), "Odometry updated - Position: [%.2f, %.2f, %.2f]",
...
void PerceptionNode::tagDetectionCallback(const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr msg)
{
  ...
  tf2::Quaternion q(
      current_imu_->orientation.x,
      current_imu_->orientation.y,
      current_imu_->orientation.z,
  ...
  geometry_msgs::msg::Point adjusted_position = calculateAdjustedPosition(
      current_odom_->pose.pose.position, yaw, depth_estimate);
  ...
  bottom_image_sub_ = std::make_shared<image_transport::Subscriber>(
      image_transport::create_subscription(
          this, "/" + drone_namespace_ + "/bottom/image",
  ...
  RCLCPP_INFO(this->get_logger(), "Scenario detected: %s at position [%.2f, %.2f, %.2f]",
              scenarioToString(detected_scenario).c_str(),
              adjusted_position.x, adjusted_position.y, adjusted_position.z);
}

apriltag_msgs::msg::AprilTagDetectionArray::UniquePtr PerceptionNode::detectAprilTags(
...
geometry_msgs::msg::Point PerceptionNode::calculateAdjustedPosition(
...
void PerceptionNode::publishScenarioDetection(
    ...
    double drone_heading)
{
  ...
  scenario_msg.data = scenarioToString(scenario_type) + "," +
                      std::to_string(target_position.x) + "," +
                      std::to_string(target_position.y) + "," +
                      std::to_string(target_position.z) + "," +
